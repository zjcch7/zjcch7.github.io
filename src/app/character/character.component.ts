...
export class CharacterComponent implements OnInit {
  ...
  constructor(
    public service: ListServiceService,  
    public fb: FormBuilder, 
    private titleService: Title, 
    private loginService: LoginService, 
    private router: Router 
  ) {
    titleService.setTitle("Create a Character - FictionBattles");
  }
  ...
  console.log(this.subscribedName)
  this.loginService.currentUserLoggedIn$.subscribe(status => this.userLoggedIn = status);

    this.service.GetUserList();
    this.characterSub = this.service.getCharacterUpdateListener().subscribe((characters: Character[]) => {
      this.characterList = characters; // Directly assign the list instead of pushing elements
    });
  }
  ...
  onAddCharacter(form: NgForm){
    ...
    const filteredName = this.filterProfanity(form.value.name);
    const filteredSource = this.filterProfanity(form.value.source);
    const filteredPowers = this.filterProfanity(form.value.powers);
    if(filteredName.includes('*')){   
      this.badwordMessage=('Abusive language is not allowed.')
      form.resetForm()
      return;
    }
    ...
    this.service.AddCharacter(filteredName, filteredSource, form.value.genre, filteredPowers,form.value.rank)
    form.resetForm()
