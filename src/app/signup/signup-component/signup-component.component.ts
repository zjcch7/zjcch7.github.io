...
import { passwordMatch } from 'src/Validators/passwordValidation';
...
export class SignupComponentComponent {
  ...
  constructor(public service: ListServiceService, public fb: FormBuilder, private titleService: Title, private dialogRef : MatDialog, private router:Router){
    titleService.setTitle("Sign Up - FictionBattles")
  }
  ngOnInit(){
  ...
  onSignUp(form: NgForm){
    ...
    const filteredName = this.filterProfanity( form.value.username);
    ...
    if (filteredName.includes("*")){
      this.badwordMessage=('Username can not inculde abusive language.')
      form.resetForm()
      return;
    }
    if(makePassword != confirmPassword){
      this.passwordMatchError='passwords must match'
      form.resetForm()
      return;
    }
    this.service.TrySignUp(form.value.email, filteredName, form.value.password);
  ...
  signupForm = new FormGroup({
    ...
    username : new FormControl("", [Validators.required, Validators.maxLength(32), Validators.minLength(4)]),
  ...
  tosPopup(){
    this.dialogRef.open(tosComponent)
  }
